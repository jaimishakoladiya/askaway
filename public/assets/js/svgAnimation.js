var activeElement, curEmailIndex, screenCenter, svgCoords, emailCoords, firstCoords, lastCoords, emailScrollMax, chinMin = .5, dFromC, mouthStatus = "small", blinking, eyeScale = 1, eyesCovered = false, showPasswordClicked = false;
...
var firstName = document.querySelector('.form-input-first');
var lastName = document.querySelector('.form-input-last');
var password = document.querySelector('.form-input-password');
var showPasswordCheck = document.querySelector('.form-input-checkPassword');
var showPasswordToggle = document.querySelector('.form-input-checkPassword');
...
function calculateFaceMove(e, element, cords) {
	var
		carPos = element.selectionEnd,
	...
	if (element.scrollWidth <= emailScrollMax) {
		caretCoords = getPosition(span);
		dFromC = screenCenter - (caretCoords.x + cords.x);
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, cords.x + caretCoords.x, cords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, cords.x + caretCoords.x, cords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, cords.x + caretCoords.x, cords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, cords.x + caretCoords.x, cords.y + 25);
	} else {
		eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, cords.x + emailScrollMax, cords.y + 25);
		eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, cords.x + emailScrollMax, cords.y + 25);
		noseAngle = getAngle(noseCoords.x, noseCoords.y, cords.x + emailScrollMax, cords.y + 25);
		mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, cords.x + emailScrollMax, cords.y + 25);
	}
...
function onEmailInput(e) {
	calculateFaceMove(e, email, emailCoords);
...
function onFirstInput(e) {
	calculateFaceMove(e, firstName, firstCoords);	
	var value = firstName.value;
	curFirstIndex = value.length;
...
function onLastInput(e) {
	calculateFaceMove(e, lastName, lastCoords);
	var value = lastName.value;
	curLastIndex = value.length;
...
function initLoginForm() {
	...
	firstCoords = getPosition(firstName);
	lastCoords = getPosition(lastName);
	...
	// handle events for firstName and lastName input
	if (firstName) {
	...
	if (password) {
		password.addEventListener('change', onPasswordToggleChange);
		password.addEventListener('focus', onPasswordToggleFocus);
		password.addEventListener('blur', onPasswordToggleBlur);
		password.addEventListener('click', onPasswordToggleClick);
		password.addEventListener('mouseup', onPasswordToggleMouseUp);
		password.addEventListener('mousedown', onPasswordToggleMouseDown);
	}
